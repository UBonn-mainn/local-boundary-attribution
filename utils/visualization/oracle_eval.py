...
def plot_2d_boundary_comparison(
    ...
    if show_gs_sphere and gs_radius is not None and np.isfinite(gs_radius) and gs_radius > 0:
        ax.add_patch(
            plt.Circle((x[0], x[1]), float(gs_radius), fill=False, linewidth=1.0, label=gs_circle_label, edgecolor="yellow")
        )

    # FGSM circle centered at x and passing through b_fgsm
    if fgsm_circle and r_fgsm is not None and np.isfinite(r_fgsm) and r_fgsm > 0:
        ax.add_patch(
            plt.Circle((x[0], x[1]), float(r_fgsm), fill=False, linewidth=1.0, label=b_circle_label, edgecolor="yellow")
        )
