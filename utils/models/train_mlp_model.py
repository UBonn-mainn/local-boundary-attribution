...
def train(args):
    ...
    model = SimpleClassifier(input_dim=input_dim, num_classes=2) # Binary classification hardcoded for now
    model.to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a simple classifier on CSV data.")
    parser.add_argument("--data_path", type=str, required=True, help="Path to input CSV file")
    parser.add_argument("--save_path", type=str, default="models/checkpoints/model.pth", help="Path to save the trained model")
