...
def main():
    parser = argparse.ArgumentParser(
        description="Run boundary search + attribution evaluation for MNIST."
    )
    ...
    if len(gs_rows) < len(samples):
        logger.warning("GS CSV has fewer rows than collected samples: %d < %d", len(gs_rows), len(samples))
    ...
    def run_search(x_input: torch.Tensor):
        ...
        return b_point_t, bool(res.success)
    ...
    # n = min(len(samples), len(gs_rows))
    n = len(gs_rows)

    for i in tqdm(range(n)):
        gs_row = gs_rows[i]
        x_img, y_i = gs_row["x0"], gs_row["class"]

        x_input = torch.tensor(x_img, dtype=torch.float32, device=device)
        ...
        gs_res = GSOracleResult(
            ...
            radius_found=float(gs_row["dist_x0_boundary"]) if np.isfinite(gs_row["dist_x0_boundary"]) else float("inf"),
        ...
        crawler_dist = float(np.linalg.norm(b_point_np.reshape(-1) - x_np_flat))
        ...
        if gs_res.success and gs_res.x_boundary is not None:
            gs_base = _np_to_torch_image(gs_res.x_boundary, device=device)
        ...
        row: Dict[str, Any] = {
            ...
            "gs_radius_found": safe_float(gs_res.radius_found),
            "dist_x_to_crawler": crawler_dist if crawler_success else np.nan,
            "dist_x_to_gs": safe_float(gs_row["dist_x0_boundary"]) if gs_res.success else np.nan,
            "dist_crawler_to_gs": (
                l2(b_point_np, np.asarray(gs_base.detach().cpu().numpy(), dtype=np.float32))
                if (crawler_success and gs_base is not None) else np.nan
            ),
